...
# string manipultation to define the required libraries based on the DEPS variable in the rules
_DEPS := $(foreach dep,$(DEPS),$(addsuffix /$(dep),$(addprefix libraries/,$(dep))))
...
$(foreach dep,$(_DEPS),$(eval $(notdir $(dep))_DIR := $(dir $(dep))) $(eval include $(dir $(dep))rules.mk))
