...
#define TEST_LIGHTS_BLINKER_SYNC_COUNT 4
...
void test_lights_blinker_init_without_sync(void) {
  LightsBlinker blinker_1 = { 0 };
  lights_blinker_init_without_sync(&blinker_1, TEST_LIGHTS_BLINKER_DURATION_SHORT);
  TEST_ASSERT_EQUAL(SOFT_TIMER_INVALID_TIMER, blinker_1.timer_id);
  LightsBlinker blinker_2 = { 0 };
  lights_blinker_init_with_sync(&blinker_2, TEST_LIGHTS_BLINKER_DURATION_SHORT,
                                TEST_LIGHTS_BLINKER_SYNC_COUNT);
  TEST_ASSERT_EQUAL(TEST_LIGHTS_BLINKER_INITIAL_COUNT, blinker_2.blink_count);
  TEST_ASSERT_EQUAL(TEST_LIGHTS_BLINKER_SYNC_COUNT, blinker_2.sync_count);
}
...
void test_lights_blinker_activate(void) {
  ...
  TEST_ASSERT_OK(lights_blinker_init_without_sync(&blinker_1, TEST_LIGHTS_BLINKER_DURATION_SHORT));
  TEST_ASSERT_OK(lights_blinker_init_without_sync(&blinker_2, TEST_LIGHTS_BLINKER_DURATION_LONG));
...
void test_lights_blinker_activate_already_active(void) {
  ...
  Event e = { 0 };
  TEST_ASSERT_OK(lights_blinker_init_without_sync(&blinker, TEST_LIGHTS_BLINKER_DURATION_SHORT));
...
void test_lights_blinker_sync_update(void) {
  ...
  // Syncing blinker.
  TEST_ASSERT_OK(lights_blinker_sync(&blinker));
  TEST_ASSERT_OK(event_process(&e));
  // Raised event must be on again. If we didn't call lights_blinker_sync, event would not get
  // raised immediately, and would have been an off blink event.
  TEST_ASSERT_EQUAL(LIGHTS_EVENT_GPIO_PERIPHERAL_SIGNAL_LEFT, e.data);
  TEST_ASSERT_EQUAL(LIGHTS_EVENT_GPIO_ON, e.id);
}

void test_lights_blinker_sync_initialized_not_activated(void) {
  // Initialized blinkers are inactive by default. They cannot be synced to ON state.
  LightsBlinker blinker = { 0 };
  TEST_ASSERT_OK(lights_blinker_init_without_sync(&blinker, TEST_LIGHTS_BLINKER_DURATION_SHORT));
  StatusCode s = lights_blinker_sync(&blinker);
  TEST_ASSERT_EQUAL(STATUS_CODE_INVALID_ARGS, s);
}

void test_lights_blinker_sync_while_deactivated(void) {
...
void test_lights_blinker_with_sync(void) {
  ...
  LightsBlinker blinker = { 0 };
  LightsBlinkerSyncCount count = 3;
  TEST_ASSERT_OK(
      lights_blinker_init_with_sync(&blinker, TEST_LIGHTS_BLINKER_DURATION_SHORT, count));
  TEST_ASSERT_OK(lights_blinker_activate(&blinker, LIGHTS_EVENT_GPIO_PERIPHERAL_SIGNAL_LEFT));
  Event e = { 0 };
  TEST_ASSERT_OK(event_process(&e));
  TEST_ASSERT_EQUAL(LIGHTS_EVENT_GPIO_ON, e.id);
