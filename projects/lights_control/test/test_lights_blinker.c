...
void test_lights_blinker_activate(void) {
  ...
  TEST_ASSERT_OK(lights_blinker_activate(&blinker_1, LIGHTS_EVENT_SIGNAL_HAZARD));
  ...
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_HAZARD_BLINK_ON);

  TEST_ASSERT_OK(lights_blinker_activate(&blinker_2, LIGHTS_EVENT_SIGNAL_LEFT));
  ...
  TEST_ASSERT_OK(event_process(&e));
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_LEFT_BLINK_ON);

  // Now we make sure blinker 1 timer goes off before blinker 2
  while (event_process(&e) != STATUS_CODE_OK) {
  }
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_HAZARD_BLINK_OFF);

  // Wait for the second blinker
  while (event_process(&e) != STATUS_CODE_OK) {
  }
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_LEFT_BLINK_OFF);
}
...
void test_lights_blinker_activate_already_active(void) {
  ...
  TEST_ASSERT_OK(lights_blinker_activate(&blinker, LIGHTS_EVENT_SIGNAL_LEFT));
  TEST_ASSERT_OK(event_process(&e));
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_LEFT_BLINK_ON);
  TEST_ASSERT_OK(lights_blinker_activate(&blinker, LIGHTS_EVENT_SIGNAL_RIGHT));
  TEST_ASSERT_OK(event_process(&e));
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_LEFT_BLINK_OFF);
  TEST_ASSERT_OK(event_process(&e));
  TEST_ASSERT_EQUAL(e.id, LIGHTS_EVENT_SIGNAL_RIGHT_BLINK_ON);
}

void test_lights_blinker_activate_with_existing_event(void) {
  // Activating an already-active blinker with the same event will have no effect on the behaviour.
  LightsBlinker blinker = { 0 };
  Event e = { 0 };
  TEST_ASSERT_OK(lights_blinker_init(&blinker, TEST_LIGHTS_BLINKER_DURATION_SHORT));
  TEST_ASSERT_OK(lights_blinker_activate(&blinker, LIGHTS_EVENT_SIGNAL_LEFT));
