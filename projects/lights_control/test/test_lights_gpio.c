...
void test_lights_gpio_process_event_1(void) {
  ...
  const Event test_event = {
    .id = TEST_LIGHTS_GPIO_MOCK_EVENT_1,
    .data = LIGHTS_GPIO_STATE_ON
  };
  TEST_ASSERT_OK(lights_gpio_process_event(&s_mock_config, &test_event));
  GPIOState gpio_state;
  TEST_ASSERT_OK(gpio_get_state(&s_mock_peripherals[TEST_LIGHTS_GPIO_MOCK_PERIPHERAL_1].address, &gpio_state));
  TEST_ASSERT_EQUAL(gpio_state, GPIO_STATE_LOW);
  TEST_ASSERT_OK(gpio_get_state(&s_mock_peripherals[TEST_LIGHTS_GPIO_MOCK_PERIPHERAL_3].address, &gpio_state));
...
void test_lights_gpio_process_event_2(void) {
  ...
  const Event test_event = {
    .id = TEST_LIGHTS_GPIO_MOCK_EVENT_1,
    .data = LIGHTS_GPIO_STATE_OFF
  };
  TEST_ASSERT_OK(lights_gpio_process_event(&s_mock_config, &test_event));
  GPIOState gpio_state;
  TEST_ASSERT_OK(gpio_get_state(&s_mock_peripherals[TEST_LIGHTS_GPIO_MOCK_PERIPHERAL_2].address, &gpio_state));
  TEST_ASSERT_EQUAL(gpio_state, GPIO_STATE_HIGH);
}
...
void test_lights_gpio_process_event_3(void) {
  // As can be seen in s_mock_event_mappings, event 1 is associated with peripherals 1 and 3.
  const Event test_event = {
    .id = TEST_LIGHTS_GPIO_MOCK_EVENT_3,
    .data = LIGHTS_GPIO_STATE_OFF
  };
  TEST_ASSERT_OK(lights_gpio_process_event(&s_mock_config, &test_event));
  GPIOState gpio_state;
  TEST_ASSERT_OK(gpio_get_state(&s_mock_peripherals[TEST_LIGHTS_GPIO_MOCK_PERIPHERAL_4].address, &gpio_state));
  TEST_ASSERT_EQUAL(gpio_state, GPIO_STATE_LOW);
  TEST_ASSERT_OK(gpio_get_state(&s_mock_peripherals[TEST_LIGHTS_GPIO_MOCK_PERIPHERAL_5].address, &gpio_state));
  TEST_ASSERT_EQUAL(gpio_state, GPIO_STATE_HIGH);
}
