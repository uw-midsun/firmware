...
StatusCode lights_strobe_init(LightsStrobe *lights_strobe,
                LightsBlinkerDuration blinker_duration) {
  status_ok_or_return(lights_blinker_init(&lights_strobe->blinker, blinker_duration));
}

StatusCode lights_strobe_process_event(LightsStrobe *lights_strobe, const Event *event) {
  if (event->id == LIGHTS_EVENT_STROBE_ON) {
    status_ok_or_return(lights_blinker_activate(&lights_strobe->blinker,
