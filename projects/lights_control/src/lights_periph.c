...
static GPIOSettings s_settings = { .direction = GPIO_DIR_OUT,
...
StatusCode lights_periph_simple(Event e) {
  if (s_boardtype == LIGHTS_BOARD_FRONT) {
    return gpio_set_state(&s_front_address_lookup[e.id],
                          (e.data) ? GPIO_STATE_HIGH : GPIO_STATE_LOW);
  } else if (s_boardtype == LIGHTS_BOARD_REAR) {
    return gpio_set_state(&s_rear_address_lookup[e.id],
                          (e.data) ? GPIO_STATE_HIGH : GPIO_STATE_LOW);
  }
