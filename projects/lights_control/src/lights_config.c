...
StatusCode lights_config_init(void) {
  ...
  LightsConfigBoardType board_type =
      (state == GPIO_STATE_HIGH) ? LIGHTS_CONFIG_BOARD_TYPE_FRONT : LIGHTS_CONFIG_BOARD_TYPE_REAR;
  ...
  s_can_settings.device_id = (board_type == LIGHTS_CONFIG_BOARD_TYPE_FRONT)
                                 ? SYSTEM_CAN_DEVICE_SOLAR_MASTER_FRONT
                                 : SYSTEM_CAN_DEVICE_SOLAR_MASTER_REAR;
