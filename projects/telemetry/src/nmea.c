...
NMEAResult parse_nmea_sentence(const uint8_t *rx_arr, size_t len){
  ...
  if(len < 1 && rx_arr[0] != '$'){
    LOG_WARN("Something is fishy, first character of a NMEA sentence should be a $, it is a %c", rx_arr[0]);
  }
  if(len < 3 && (rx_arr[1] != 'G' || rx_arr[2] != 'P')){
    LOG_WARN("Something is fishy, the second and third characters of a NMEA sentence should be a GP. They are %c%c", rx_arr[1], rx_arr[2]);
  }
  ...
  if(m_id == GGA){
    ...
    sscanf(temp_buf[1], "%2d%2d%4d", (int *) &r.gga.latitude.degrees, (int *) &r.gga.latitude.minutes, (int *) &r.gga.latitude.fraction);
    r.gga.north_south = (uint8_t) temp_buf[2][0];
    sscanf(temp_buf[3], "%2d%2d%4d", (int *) &r.gga.longtitude.degrees, (int *) &r.gga.longtitude.minutes, (int *) &r.gga.longtitude.fraction);
