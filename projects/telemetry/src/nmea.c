...
NMEAResult parse_nmea_sentence(const uint8_t *rx_arr, size_t len) {
  ...
  if (realcheck != checksum) {
    LOG_WARN("Checksums do not match, received: %d, calculated: %d",
    (int) realcheck, (int) checksum);
  }
  // Parse message_id below

  char temp_buf[GGA][10] = {{0}};
  int32_t b1 = 0; // This is on purpose, so it does not underflow.
  uint32_t b = 0; // It will still works if b1 underflows (because it will immediately overflow)?
  ...
  if (m_id == GGA) {
    ...
    sscanf(temp_buf[1], "%2d%2d%4d", (int *) &r.gga.latitude.degrees, \
           (int *) &r.gga.latitude.minutes, (int *) &r.gga.latitude.fraction);
