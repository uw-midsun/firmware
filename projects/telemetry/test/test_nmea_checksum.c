...
void test_compute_checksum(void) {
  ...
  TEST_ASSERT(nmea_compute_checksum(message, message_len, computed_checksum,
                                    sizeof(computed_checksum)) == STATUS_CODE_OK);
  ...
  message_len = strlen(message);
  char small_out[2];
  TEST_ASSERT(nmea_compute_checksum(message, message_len, small_out, sizeof(small_out)) ==
