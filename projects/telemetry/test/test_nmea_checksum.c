...
void test_compute_checksum(void) {
  ...
  TEST_ASSERT(nmea_compute_checksum(message, message_len, &computed_checksum) == STATUS_CODE_OK);
  ...
  message = "$GPGLL,2503.6319,N,12136.0099,E,053740.000,A,A*52";
  message_len = strlen(message) + 1;
  TEST_ASSERT(nmea_compute_checksum(message, message_len, NULL) == STATUS_CODE_INVALID_ARGS);
}
