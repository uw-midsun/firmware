...
static void prv_clock_expected_lights(TestSignalsExpectedLight *lights, size_t num_lights) {
  ...
  while (found_bitset != expected_bitset) {
    ...
    if (e.id == INPUT_EVENT_CENTER_CONSOLE_CAN_RX) {
...
void setup_test(void) {
  ...
  CanSettings can_settings = {
    ...
    .tx_event = INPUT_EVENT_CENTER_CONSOL_CAN_TX,
...
void test_signals_turn(void) {
  ...
  TEST_SIGNALS_CLOCK_EVENT(INPUT_EVENT_CENTER_CONSOLE_CONTROL_STALK_ANALOG_TURN_SIGNAL_LEFT, true);
  prv_clock_expected_lights(expected_lights, SIZEOF_ARRAY(expected_lights));

  // New turn signal overrides previous
  TEST_SIGNALS_CLOCK_EVENT(INPUT_EVENT_CENTER_CONSOLE_CONTROL_STALK_ANALOG_TURN_SIGNAL_RIGHT, true);
  ...
  TEST_SIGNALS_CLOCK_EVENT(INPUT_EVENT_CENTER_CONSOLE_POWER_STATE_OFF, true);
...
void test_signals_headlights(void) {
  ...
  TEST_SIGNALS_CLOCK_EVENT(INPUT_EVENT_CENTER_CONSOLE_POWER_STATE_FAULT, true);
  expected_lights[1].state = EE_LIGHT_STATE_OFF;
  prv_clock_expected_lights(expected_lights, SIZEOF_ARRAY(expected_lights));
}
