...
void test_sequencer_transitions(void) {
  // Valid: idle -> charge
  Event event = { .id = CHAOS_EVENT_SEQUENCE_CHARGE };
  TEST_ASSERT_OK(sequencer_publish_next_event(&event));

  // Invalid: charge -> drive
  event.id = CHAOS_EVENT_SEQUENCE_DRIVE;
  TEST_ASSERT_EQUAL(STATUS_CODE_INVALID_ARGS, sequencer_publish_next_event(&event));
