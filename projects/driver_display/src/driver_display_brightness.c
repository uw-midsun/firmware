...
void driver_display_brightness_calibration(GPIOAddress adc_address) {
  ...
  uint16_t reading;
  soft_timer_start_seconds(DRIVER_DISPLAY_CONFIG_REFRESH_PERIOD, driver_display_brightness_timer_callback, (void*)&reading, DRIVER_DISPLAY_CONFIG_REFRESH_TIMER);
  ...
  printf("Min: %d, Max: %d, Range: %d", data.min_brightness, data.max_brightness, data.range_brightness);
  printf("\n");
}
...
void driver_display_brightness_read(GPIOAddress adc_address) {
  ...
  uint16_t percent_reading =
      ((reading - data.min_brightness) * 100) / data.range_brightness;
