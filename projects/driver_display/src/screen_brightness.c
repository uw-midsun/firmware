...
static const GPIOSettings gpio_settings = { GPIO_DIR_OUT, GPIO_STATE_LOW, GPIO_ALTFN_NONE,
                                            GPIO_RES_NONE };

static const GPIOSettings adc_settings = { GPIO_DIR_IN, GPIO_STATE_LOW, GPIO_RES_NONE,
                                           GPIO_ALTFN_ANALOG };
...
void read_brightness(void) {
  ...
  for (;;) {  // infinite for loop
    adc_read_raw(adc_channel,
                 &reading);  // adc_register_callback(adc_channel, test_callback, &reading)
