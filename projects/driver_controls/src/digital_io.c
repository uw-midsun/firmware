...
static uint16_t prv_get_event(DigitalIOData data, GPIOState state) {
  ...
  if (data.id == DIGITAL_IO_DEVICE_DIRECTION_SELECTOR &&
      state == GPIO_STATE_HIGH) {
    event_id = INPUT_EVENT_DIRECTION_SELECTOR_NEUTRAL;
  } else if (data.id == DIGITAL_IO_DEVICE_TURN_SIGNAL &&
             state == GPIO_STATE_HIGH) {
...
void digital_io_init(void) {
  ...
  s_input_data[DRIVER_IO_POWER_SWITCH_PIN] = ((DigitalIOData){
      .id = DIGITAL_IO_DEVICE_POWER_SWITCH, .event = INPUT_EVENT_POWER});
  ...
  s_input_data[DRIVER_IO_CRUISE_CONTROL_PIN] =
      ((DigitalIOData){.id = DIGITAL_IO_DEVICE_CRUISE_CONTROL,
  ...
  s_input_data[DRIVER_IO_TURN_SIGNAL_PIN_RIGHT] =
      ((DigitalIOData){.id = DIGITAL_IO_DEVICE_TURN_SIGNAL,
  ...
  s_input_data[DRIVER_IO_HAZARD_LIGHT_PIN] = ((DigitalIOData){
      .id = DIGITAL_IO_DEVICE_HAZARD_LIGHT, .event = INPUT_EVENT_HAZARD_LIGHT});

  s_input_data[DRIVER_IO_HORN_PIN] = ((DigitalIOData){
      .id = DIGITAL_IO_DEVICE_HORN, .event = INPUT_EVENT_HORN});

  s_input_data[DRIVER_IO_PUSH_TO_TALK_PIN] = ((DigitalIOData){
      .id = DIGITAL_IO_DEVICE_PUSH_TO_TALK, .event = INPUT_EVENT_PUSH_TO_TALK});
  ...
  DigitalIOSettings digital_inputs[] = {
      ...
      {.address = DRIVER_IO_DIR_SELECT_FORWARD,
       .edge = INTERRUPT_EDGE_RISING_FALLING},
      {.address = DRIVER_IO_DIR_SELECT_REVERSE,
       .edge = INTERRUPT_EDGE_RISING_FALLING},
  ...
  for (uint8_t i = 0; i < SIZEOF_ARRAY(digital_inputs); i++) {
    ...
    gpio_it_register_interrupt(&digital_inputs[i].address, &it_settings,
                               digital_inputs[i].edge, prv_input_callback,
                               &s_input_data[pin]);
  }
}
