...
static void prv_state_off(FSM *fsm, const Event *e, void *context) {
  ...
  *event_check = prv_check_off;
  ...
  event_arbiter_can_output(CAN_OUTPUT_DEVICE_ID_POWER, POWER_FSM_STATE_OFF, e->data);
}
...
static void prv_state_on(FSM *fsm, const Event *e, void *context) {
  ...
  *event_check = NULL;
  ...
  event_arbiter_can_output(CAN_OUTPUT_DEVICE_ID_POWER, POWER_FSM_STATE_ON, e->data);
}
