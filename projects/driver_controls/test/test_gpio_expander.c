...
void test_gpio_expander_init_pin(void) {
  ...
  TEST_ASSERT_NOT_OK(
      gpio_expander_init_pin(NUM_GPIO_EXPANDER_PINS, &output_settings));
...
void test_gpio_expander_register_callback(void) {
  TEST_ASSERT_NOT_OK(gpio_expander_register_callback(NUM_GPIO_EXPANDER_PINS,
                                                     prv_callback, NULL));

  TEST_ASSERT_OK(
      gpio_expander_register_callback(GPIO_EXPANDER_PIN_0, prv_callback, NULL));
  TEST_ASSERT_OK(
      gpio_expander_register_callback(GPIO_EXPANDER_PIN_1, prv_callback, NULL));
}
...
void test_gpio_expander_set_state(void) {
  ...
  TEST_ASSERT_NOT_OK(
      gpio_expander_set_state(NUM_GPIO_EXPANDER_PINS, GPIO_STATE_LOW));
  TEST_ASSERT_NOT_OK(
      gpio_expander_set_state(GPIO_EXPANDER_PIN_0, GPIO_STATE_LOW));
