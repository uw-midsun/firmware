...
FSM_STATE_TRANSITION(highbeam_off) {
  FSM_ADD_TRANSITION(INPUT_EVENT_CONTROL_STALK_DIGITAL_HIGH_BEAM_FWD_PRESSED, highbeam_on);
}

FSM_STATE_TRANSITION(highbeam_on) {
...
StatusCode highbeam_fsm_init(Fsm *fsm, EventArbiterStorage *storage) {
  fsm_state_init(highbeam_off, prv_highbeam_off_output);
  fsm_state_init(highbeam_on, prv_highbeam_on_output);
