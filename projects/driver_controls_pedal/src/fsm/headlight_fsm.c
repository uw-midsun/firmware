...
// According to lighting regulations, we technically only need these states:
//
// * Off
// * DRL
//
// But we add extra states in order to allow a more "car-like" experience
...
FSM_STATE_TRANSITION(headlight_off) {
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_LOWBEAMS, headlight_lowbeam);
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_DRL, headlight_drl);
  // FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CONTROL_STALK_DIGITAL_HEADLIGHT_FWD_PRESSED,
  // headlight_highbeam);
}

FSM_STATE_TRANSITION(headlight_lowbeam) {
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_LOWBEAMS, headlight_off);
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_DRL, headlight_drl);
  // FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CONTROL_STALK_DIGITAL_HEADLIGHT_FWD_PRESSED,
  // headlight_lowbeam_highbeam);

  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_POWER_STATE_OFF, headlight_off);
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_POWER_STATE_FAULT, headlight_off);
}

FSM_STATE_TRANSITION(headlight_drl) {
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_LOWBEAMS, headlight_lowbeam);
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_DRL, headlight_off);
  // FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CONTROL_STALK_DIGITAL_HEADLIGHT_FWD_PRESSED,
...
FSM_STATE_TRANSITION(headlight_lowbeam_highbeam) {
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_LOWBEAMS, headlight_highbeam);
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_DRL, headlight_drl_highbeam);
  // FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CONTROL_STALK_DIGITAL_HEADLIGHT_FWD_RELEASED,
  // headlight_lowbeam);
...
FSM_STATE_TRANSITION(headlight_drl_highbeam) {
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_LOWBEAMS, headlight_lowbeam_highbeam);
  FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CENTER_CONSOLE_DRL, headlight_highbeam);
  // FSM_ADD_TRANSITION(PEDAL_EVENT_INPUT_CONTROL_STALK_DIGITAL_HEADLIGHT_FWD_RELEASED,
  // headlight_drl);
...
static void prv_headlight_off_output(Fsm *fsm, const Event *e, void *context) {
  ...
  event_raise(PEDAL_EVENT_INPUT_HEADLIGHT_STATE_OFF, 0);
}
...
static void prv_headlight_lowbeam_output(Fsm *fsm, const Event *e, void *context) {
  ...
  event_raise(PEDAL_EVENT_INPUT_HEADLIGHT_STATE_LOWBEAM, 0);
}
...
static void prv_headlight_drl_output(Fsm *fsm, const Event *e, void *context) {
  ...
  event_raise(PEDAL_EVENT_INPUT_HEADLIGHT_STATE_DRL, 0);
}
...
static void prv_headlight_highbeam_output(Fsm *fsm, const Event *e, void *context) {
  ...
  event_raise(PEDAL_EVENT_INPUT_HEADLIGHT_STATE_HIGHBEAM, 0);
}
