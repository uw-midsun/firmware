...
void test_ltc_afe_adc_conversion_initiated(void) {
  uint16_t voltages[LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN] = {
      0};
  StatusCode status = ltc_afe_read_all_voltage(&s_afe_settings, voltages,
                                               SIZEOF_ARRAY(voltages));
  ...
  for (int i = 0; i < LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN;
       ++i) {
...
void test_ltc_afe_read_all_voltage_repeated_within_tolerances(void) {
  ...
  uint16_t samples[LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN *
                   AFE_TEST_SAMPLES] = {0};
  ...
  for (int sample = 0; sample < AFE_TEST_SAMPLES; ++sample) {
    ...
    for (int cell = 0;
         cell < LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN;
         ++cell) {
      samples[sample * LTC6804_CELLS_PER_DEVICE + cell] = voltages[cell];

      for (int old_sample = 0; old_sample < sample; ++old_sample) {
        uint16_t old_value =
            samples[sample * LTC6804_CELLS_PER_DEVICE + old_sample];
        TEST_ASSERT_TRUE(prv_within_range(old_value, voltages[cell],
                                          AFE_TEST_VOLTAGE_VARIATION));
      }
    }
  }
}
...
void test_ltc_afe_read_all_aux_repeated_within_tolerances(void) {
  ...
  for (int sample = 0; sample < AFE_TEST_SAMPLES; ++sample) {
    uint16_t voltages[LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN] =
        {0};
    StatusCode status =
        ltc_afe_read_all_aux(&s_afe_settings, voltages, SIZEOF_ARRAY(voltages));
    TEST_ASSERT_OK(status);
...
void test_ltc_afe_toggle_discharge_cells_valid_range(void) {
  ...
  StatusCode status =
      ltc_afe_toggle_discharge_cells(&s_afe_settings, valid_cell, true);

  TEST_ASSERT_OK(status);
}

void test_ltc_afe_toggle_discharge_cells_invalid_range(void) {
  uint16_t invalid_cell =
      LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN;
  StatusCode status =
      ltc_afe_toggle_discharge_cells(&s_afe_settings, invalid_cell, true);
