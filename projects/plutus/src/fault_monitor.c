...
StatusCode fault_monitor_check(const FaultMonitorSettings *settings, FaultMonitorResult *result) {
  ...
  StatusCode status =
      ltc_afe_read_all_voltage(settings->ltc_afe, result->cell_voltages, PLUTUS_CFG_AFE_TOTAL_CELLS);
  ...
  status = ltc_afe_read_all_aux(settings->ltc_afe, result->temp_voltages, PLUTUS_CFG_AFE_TOTAL_CELLS);
  if (status != STATUS_CODE_OK) {
  ...
  for (size_t i = 0; i < PLUTUS_CFG_AFE_TOTAL_CELLS; i++) {
    ...
    had_fault |= (result->cell_voltages[i] < settings->undervoltage || result->cell_voltages[i] > settings->overvoltage);
  }
