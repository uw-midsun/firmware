...
static bool s_discharging_cells[LTC6804_CELLS_PER_DEVICE *
                                PLUTUS_AFE_DEVICES_IN_CHAIN] = {false};
...
static uint8_t s_voltage_reg[NUM_LTC_AFE_VOLTAGE_REGISTER] = {
    LTC_AFE_REGISTER_CELL_VOLTAGE_A, LTC_AFE_REGISTER_CELL_VOLTAGE_B,
    LTC_AFE_REGISTER_CELL_VOLTAGE_C, LTC_AFE_REGISTER_CELL_VOLTAGE_D};
...
static StatusCode prv_read_register(const LtcAfeSettings *afe,
                                    LtcAfeRegister reg, uint8_t *data,
                                    size_t len) {
...
static StatusCode prv_write_config(const LtcAfeSettings *afe,
  ...
  for (uint8_t device = PLUTUS_AFE_DEVICES_IN_CHAIN; device > 0; --device) {
    ...
    config_packet.devices[curr_device].reg.discharge_c1 =
        s_discharging_cells[cell_index + 0];
    config_packet.devices[curr_device].reg.discharge_c2 =
        s_discharging_cells[cell_index + 1];
    config_packet.devices[curr_device].reg.discharge_c3 =
        s_discharging_cells[cell_index + 2];
    config_packet.devices[curr_device].reg.discharge_c4 =
        s_discharging_cells[cell_index + 3];
    config_packet.devices[curr_device].reg.discharge_c5 =
        s_discharging_cells[cell_index + 4];
    config_packet.devices[curr_device].reg.discharge_c6 =
        s_discharging_cells[cell_index + 5];
    config_packet.devices[curr_device].reg.discharge_c7 =
        s_discharging_cells[cell_index + 6];
    config_packet.devices[curr_device].reg.discharge_c8 =
        s_discharging_cells[cell_index + 7];
    config_packet.devices[curr_device].reg.discharge_c9 =
        s_discharging_cells[cell_index + 8];
    config_packet.devices[curr_device].reg.discharge_c10 =
        s_discharging_cells[cell_index + 9];
    config_packet.devices[curr_device].reg.discharge_c11 =
        s_discharging_cells[cell_index + 10];
    config_packet.devices[curr_device].reg.discharge_c12 =
        s_discharging_cells[cell_index + 11];

    config_packet.devices[curr_device].reg.discharge_timeout =
    ...
    uint16_t cfgr_pec =
        crc15_calculate((uint8_t *)&config_packet.devices[curr_device].reg, 6);
...
StatusCode ltc_afe_read_all_voltage(const LtcAfeSettings *afe,
                                    uint16_t *result_data, size_t len) {
  ...
  for (uint8_t cell_reg = 0; cell_reg < NUM_LTC_AFE_VOLTAGE_REGISTER;
       ++cell_reg) {
    LtcAfeVoltageRegisterGroup voltage_register[PLUTUS_AFE_DEVICES_IN_CHAIN] = {
        0};
    ...
    for (uint8_t device = 0; device < PLUTUS_AFE_DEVICES_IN_CHAIN; ++device) {
      ...
      uint16_t data_pec =
          crc15_calculate((uint8_t *)&voltage_register[device], 6);
...
StatusCode ltc_afe_read_all_aux(const LtcAfeSettings *afe,
                                uint16_t *result_data, size_t len) {
  if (len != LTC6804_CELLS_PER_DEVICE * PLUTUS_AFE_DEVICES_IN_CHAIN) {
    return status_code(STATUS_CODE_INVALID_ARGS);
  }
  ...
  for (uint8_t cell = 0; cell < LTC6804_CELLS_PER_DEVICE; ++cell) {
    ...
    LTCAFEAuxRegisterGroupPacket register_data[PLUTUS_AFE_DEVICES_IN_CHAIN] = {
        0};

    size_t len = sizeof(register_data);
    prv_read_register(afe, LTC_AFE_REGISTER_AUX_A, (uint8_t *)register_data,
                      len);
