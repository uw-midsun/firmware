...
#define CAN_TRANSMIT_OVUV_DCDC_AUX(dcdc_ov_flag_u8, dcdc_uv_flag_u8,           \
                                   aux_bat_ov_flag_u8, aux_bat_uv_flag_u8)     \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_OVUV_DCDC_AUX(&msg, (dcdc_ov_flag_u8), (dcdc_uv_flag_u8),         \
                           (aux_bat_ov_flag_u8), (aux_bat_uv_flag_u8));        \
...
#define CAN_TRANSMIT_LIGHTS_STATES(hazard_u8, left_turn_u8, right_turn_u8,     \
                                   brakes_u8, headlights_u8)                   \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_LIGHTS_STATES(&msg, (hazard_u8), (left_turn_u8), (right_turn_u8), \
                           (brakes_u8), (headlights_u8));                      \
    StatusCode status = can_transmit(&msg, NULL);                              \
    status;                                                                    \
  })
...
#define CAN_TRANSMIT_BATTERY_VCT(module_id_u16, voltage_u16, current_u16,      \
                                 temperature_u16)                              \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_BATTERY_VCT(&msg, (module_id_u16), (voltage_u16), (current_u16),  \
                         (temperature_u16));                                   \
    StatusCode status = can_transmit(&msg, NULL);                              \
    status;                                                                    \
  })

#define CAN_TRANSMIT_MOTOR_CONTROLLER_VC(mc_voltage_1_u16, mc_current_1_u16,   \
                                         mc_voltage_2_u16, mc_current_2_u16)   \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_MOTOR_CONTROLLER_VC(&msg, (mc_voltage_1_u16), (mc_current_1_u16), \
                                 (mc_voltage_2_u16), (mc_current_2_u16));      \
    StatusCode status = can_transmit(&msg, NULL);                              \
    status;                                                                    \
  })
...
#define CAN_TRANSMIT_AUX_DCDC_VC(aux_voltage_u16, aux_current_u16,             \
                                 dcdc_voltage_u16, dcdc_current_u16)           \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_AUX_DCDC_VC(&msg, (aux_voltage_u16), (aux_current_u16),           \
                         (dcdc_voltage_u16), (dcdc_current_u16));              \
    StatusCode status = can_transmit(&msg, NULL);                              \
    status;                                                                    \
  })
...
#define CAN_TRANSMIT_SOLAR_DATA_FRONT(module_id_u16, voltage_u16, current_u16, \
                                      temperature_u16)                         \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_SOLAR_DATA_FRONT(&msg, (module_id_u16), (voltage_u16),            \
                              (current_u16), (temperature_u16));               \
    StatusCode status = can_transmit(&msg, NULL);                              \
    status;                                                                    \
  })

#define CAN_TRANSMIT_SOLAR_DATA_REAR(module_id_u16, voltage_u16, current_u16,  \
                                     temperature_u16)                          \
  ({                                                                           \
    CANMessage msg = {0};                                                      \
    CAN_PACK_SOLAR_DATA_REAR(&msg, (module_id_u16), (voltage_u16),             \
                             (current_u16), (temperature_u16));                \
    StatusCode status = can_transmit(&msg, NULL);                              \
    status;                                                                    \
  })
