...
static x86_interrupt_handler
    s_x86_interrupt_handlers[NUM_X86_INTERRUPT_HANDLERS];

// Signal handler for all interrupts. Prioritization is handled by the
// implementation of signals and
// the init function. Signals of higher priority interrupt the running of this
// function. All other
// signals are stored in a pqueue and are executed in order of priority then
// arrival. Runs the
...
void x86_interrupt_init(void) {
  // Assign the s_main_thread_id to be the thread "owning" the interrupts. This
  // prevents
...
StatusCode x86_interrupt_register_handler(x86_interrupt_handler handler,
                                          uint8_t *handler_id) {
