...
StatusCode can_rx_register_default_handler(CANRxHandlers *rx_handlers,
                                           CANRxHandlerCb handler,
                                           void *context) {
  StatusCode ret = can_rx_register_handler(rx_handlers, CAN_MSG_INVALID_ID,
                                           handler, context);

  if (ret == STATUS_CODE_OK) {
    rx_handlers->default_handler =
        can_rx_get_handler(rx_handlers, CAN_MSG_INVALID_ID);
  }

  return ret;
}

StatusCode can_rx_register_handler(CANRxHandlers *rx_handlers,
                                   CANMessageID msg_id, CANRxHandlerCb handler,
                                   void *context) {
  if (rx_handlers->num_handlers == rx_handlers->max_handlers) {
    ...
    return status_msg(STATUS_CODE_RESOURCE_EXHAUSTED,
                      "CAN RX handler already registered");
  }
...
CANRxHandler *can_rx_get_handler(CANRxHandlers *rx_handlers,
                                 CANMessageID msg_id) {
  const CANRxHandler key = {
      .msg_id = msg_id,
  };
  CANRxHandler *handler =
      bsearch(&key, rx_handlers->storage, rx_handlers->num_handlers,
              sizeof(*rx_handlers->storage), prv_handler_comp);
