...
void adc_init(ADCSettings* settings) {
  ...
  ADC_ContinuousModeCmd(ADC1, settings->mode);
...
uint16_t adc_read(GPIOAddress* address, uint16_t max) {
	...
	uint8_t adc_channel = prv_get_channel(address);
  uint16_t resolution = 1 << (12 - 2*adc_channel);

	if (!continuous_mode) {
		ADC_StartOfConversion(ADC1);
	}

	ADC1->CFGR1 ^= (continuous_mode) ? ADC_CR_ADSTART : 0;
  ADC1->CHSELR = 1 << adc_channel;
	ADC1->CFGR1 ^= (continuous_mode) ? ADC_CR_ADSTART : 0;
  
  uint16_t adc_reading = ADC_GetConversionValue(ADC1);
  uint16_t reading = (max * adc_reading)/resolution;
