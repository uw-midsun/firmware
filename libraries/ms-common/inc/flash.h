...
//
// Note that due to the way flash works, writes can only flip bits from 1 -> 0 and must be aligned
// In order to switch bits from 0 -> 1, the entire page must be erased
#include <stdint.h>
#include <stddef.h>
...
// Note that writes can only flip bits from 1 -> 0 - you must erase the page to switch from 0 -> 1.
// address must be aligned to FLASH_WRITE_BYTES and
