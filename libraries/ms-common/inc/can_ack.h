...
// from the list. We use an object pool and an array of pointers to do so
// efficiently.
#include "can_msg.h"
#include "objpool.h"
...
// this.
// Example: ack_request.expected_bitset = CAN_ACK_EXPECTED_DEVICES(CAN_DEVICE_A,
// CAN_DEVICE_D)
...
typedef StatusCode (*CANAckRequestCb)(CANMessageID msg_id, uint16_t device,
                                      CANAckStatus status,
...
static_assert(SIZEOF_FIELD(CANAckRequest, expected_bitset) * CHAR_BIT >=
                  CAN_MSG_MAX_DEVICES,
...
static_assert(SIZEOF_FIELD(CANAckPendingReq, expected_bitset) * CHAR_BIT >=
                  CAN_MSG_MAX_DEVICES,
              "CAN pending ACK expected bitset field not large enough to fit "
              "all CAN devices!");
static_assert(
    SIZEOF_FIELD(CANAckPendingReq, expected_bitset) ==
