...
void setup_test(void) { pqueue_backed_init(&s_queue, s_nodes, s_elems); }
...
void test_pqueue_backed_run(void) {
  ...
  for (size_t i = 0; i < SIZEOF_ARRAY(prios); i++) {
    StatusCode result =
        pqueue_backed_push(&s_queue, &(TestObject){.data = prios[i]}, prios[i]);
  ...
  for (size_t i = 0; i < SIZEOF_ARRAY(prios); i++) {
    ...
    TEST_ASSERT_EQUAL(STATUS_CODE_OK, result);
    TEST_ASSERT_MESSAGE(last_prio <= obj.data,
                        "Last data was lower than new data!");
    last_prio = obj.data;
  }
}
