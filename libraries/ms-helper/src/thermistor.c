...
static const uint32_t s_resistance_lookup[][THERMISTOR_LOOKUP_RANGE + 1] = {
  ...
  {
      357012000, 338065466, 320192061, 303340723, 287460390, 272500000, 258408490, 245134797,
      232627859, 220836614, 209710000, 199200088, 189271488, 179891944, 171029200, 162651000,
      154726534, 147230778, 140140155, 133431088, 127080000, 121064361, 115365832, 109967123,
      104850943, 100000000, 95397831,  91031277,  86888008,  82955693,  79222000,  75675125,
      72305364,  69103541,  66060479,  63167000,  60414367,  57795595,  55304140,  52933457,
      50677000,  48528424,  46482183,  44532929,  42675317,  40904000,  39213953,  37601442,
      36063054,  34595377,  33195000,  31858572,  30582994,  29365231,  28202245,  27091000,
      26028615,  25012829,  24041535,  23112628,  22224000,  21373639,  20559906,  19781253,
      19036133,  18323000,  17640315,  16986572,  16360271,  15759913,  15184000,  14631164,
      14100559,  13591473,  13103191,  12635000,  12186150,  11755751,  11342876,  10946601,
      10566000,  10200205,  9848582,   9510557,   9185555,   8873000,   8572351,   8283201,
      8005175,   7737900,   7481000,   7234086,   6996701,   6768373,   6548631,   6337000,
      6133009,   5936187,   5746059,   5562154,   5384000,
  }
};
...
StatusCode thermistor_get_resistance(ThermistorPosition position, uint16_t dividor_resistor_ohms,
  ...
  if (position == THERMISTOR_POSITION_R1) {
    *thermistor_resistance_mohms = (((uint32_t)(vdda_mv - reading_mv)) *
                                    (uint32_t)dividor_resistor_ohms / (uint32_t)reading_mv) *
                                   1000;
  } else {
    *thermistor_resistance_mohms = (((uint32_t)dividor_resistor_ohms * (uint32_t)reading_mv) /
                                    (uint32_t)(vdda_mv - reading_mv)) *
                                   1000;
  }
  return STATUS_CODE_OK;
}
...
StatusCode thermistor_calculate_temp(ThermistorModel model, uint32_t thermistor_resistance_mohms,
  ...
  for (uint16_t i = 0; i < THERMISTOR_LOOKUP_RANGE + 1; i++) {
    if (thermistor_resistance_mohms <= lookup_table[i] &&
      ...
      *temperature_dc =
          (uint16_t)(((uint32_t)i * 100 + ((lookup_table[i] - thermistor_resistance_mohms) /
  ...
  if (temperature_dc > THERMISTOR_CELSIUS_TO_DECICELSIUS(THERMISTOR_LOOKUP_RANGE)) {
    ...
    *thermistor_resistance_ohms = THERMISTOR_MILLIOHMS_TO_OHMS(
        (lookup_table[lower_temp] * 10 +
         (lookup_table[lower_temp + 1] - lookup_table[lower_temp]) * (temperature_dc % 10)) /
        10);
  }
...
StatusCode thermistor_read_and_calculate_temp(ThermistorStorage *storage,
                                              uint16_t *temperature_dc) {
  ...
  status_ok_or_return(thermistor_get_resistance(settings->position, settings->dividor_resistor_ohms,
                                                reading_mv, vdda_mv, &thermistor_resistance_mohms));
