...
uint32_t thermistor_converter_get_temp(ThermistorStorage *storage) {
  ...
  thermistor_resistance = (storage->sibling_resistance/reading )*(storage->source_voltage) - storage->sibling_resistance;
  ...
  for (uint16_t i = 0; i < RESISTANCE_LEN; i++) {
    if (thermistor_resistance <= resistance[i] && thermistor_resistance >= resistance[i + 1]) {
      ...
      return ((uint32_t)i*1000 + ((resistance[i] - thermistor_resistance)*1000 / (resistance[i] - resistance[i+1])));
    }
  }
